...
export const runtime = "edge"; // works on Cloudflare/Next-on-Pages
...
// Simple sanitizer (no replaceAll for Cloudflare’s runtime)
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!name || !email || !message) {
      return new Response(
        JSON.stringify({ ok: false, error: "Missing name, email, or message" }),
        { status: 400, headers: { "content-type": "application/json" } }
      );
    }
...
`;
    ...
    // ✅ Use correct property names for Resend’s latest version
    ...
    return new Response(
      JSON.stringify({ ok: true, message: "Message sent! We typically respond within 24 hours." }),
